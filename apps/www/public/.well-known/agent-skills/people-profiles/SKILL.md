...
- Необязательный `photo` содержит `src` на публичном media-origin и фактические размеры файла `width`/`height`. Необязательный `photo.source` содержит название `label` и ссылку `url` источника. Markdown-карточка тоже показывает фотографию и указанный источник; в `body_markdown` портрет не дублируется. При отсутствии источника не приписывай фотографию организации из `company`.

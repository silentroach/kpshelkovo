...
<div class="space-y-6">
  ...
  {#if showMap}
    <section id={mapid} class="space-y-4" data-testid="filtered-map">
      {#await import('./SettlementMap.svelte')}
        <p class="ui-shell p-6 text-sm text-muted-foreground" role="status">
          Загружаем карту…
        </p>
      {:then { default: SettlementMap }}
        <SettlementMap settlements={mapSettlements} />
      {:catch}
        <p class="ui-shell p-6 text-sm text-danger-text" role="alert">

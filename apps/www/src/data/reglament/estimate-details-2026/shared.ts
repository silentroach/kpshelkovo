...
import type { EstimateRow, NonEmptyReadonlyArray } from '@/lib/reglament/schema';

type DetailSourceOptions = Pick<EstimateDetailSourceRef, 'quote' | 'quote_items' | 'note'>;
...
const detailSourcePdfPagesTotal: Partial<Record<EstimateDetailSourcePdf, number>> = {
  final: 2,
  cleaning: 27,
  landscaping: 22,
  improvement: 18,
...
const estimateRowsById = new Map(
  flattenRows(estimate2026.sections.flatMap((section) => section.rows)).map((row) => [row.id, row])
);

const estimateSectionsById = new Map(estimate2026.sections.map((section) => [section.id, section]));

const aggregateTotalFor = (estimateRowId: string, costBucket: EstimateDetailCostBucket): number => {
  if (costBucket === 'other_cost') {
    throw new Error(`aggregate total is unavailable for other_cost control ${estimateRowId}`);
  }
...
const statusForControlTotal = (
  ...
  if (sourceTotal === null) {
    return input.status === 'needs_check'
      ? curatedStatus
      : detailNeedsCheckStatus('В источнике контроля нет суммы для сверки.', input.source_refs);
  }
  ...
  const deltas: readonly (readonly [string, number])[] = [
    ...
    ...(aggregateTotal === undefined
      ? []
      : [
          ['агрегированная смета и источник', round2(aggregateTotal - sourceTotal)] as const,
          ['детализация и агрегированная смета', round2(detailTotal - aggregateTotal)] as const
        ])
  ] as const;
  const mismatches = deltas.filter(([, delta]) => Math.abs(delta) > input.tolerance_rub);
  ...
  const reason = mismatches
    .map(([label, delta]) => `${label} расходятся на ${delta > 0 ? '+' : ''}${delta} ₽`)
    .join('; ');
...
export const detailSource = (
  ...
  if (!Number.isInteger(page) || page < 1) {
    throw new Error('страница источника детализации должна быть положительным целым числом');
  }
  if (!fragment.trim()) {
    throw new Error('фрагмент источника детализации не должен быть пустым');
  }
  if (options.quote_items?.length === 0) {
    throw new Error('позиции цитаты источника детализации не должны быть пустыми');
  }
...
export const detailSourceQuoteItem = (
  ...
  if (!input.label.trim()) {
    throw new Error('название позиции цитаты источника детализации не должно быть пустым');
  }
  if (input.resource_ids?.some((resourceId) => !resourceId.trim())) {
    throw new Error('ID ресурсов позиции цитаты источника детализации не должны быть пустыми');
  }
...
export const detailQuantity = (
  ...
  if (value !== null && unit === null) {
    throw new Error('единица измерения детализации обязательна, если значение известно');
  }
...
export const detailWorkItem = (input: EstimateDetailWorkItem): EstimateDetailWorkItem => input;

export const detailResource = (input: EstimateDetailResource): EstimateDetailResource => input;
...
export const resolveSectionControlTotals = (
  ...
  return inputs.map((input) => {
    const detailTotal = round2(
      sum(
        input.resource_ids.map((resourceId) => {
          ...
          if (!resource) {
            throw new Error(`resource ${resourceId} is missing for control ${input.id}`);
          }

          const total = resource.total_rub.value;

          if (total === null) {
            throw new Error(`resource ${resource.id} has no total for control ${input.id}`);
          }
    ...
    return {
      ...
      delta_rub: aggregateTotal === undefined ? undefined : round2(detailTotal - aggregateTotal),

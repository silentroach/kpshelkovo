...
const allRows = flattenRows(estimate2026.sections.flatMap((section) => section.rows));
...
const aggregateGrossById = new Map<string, number>([
  ...
  ...allRows.map((row): readonly [string, number] => [row.id, row.baseline.annual_gross])
]);

const detailGrossById = new Map<string, number>([
  [
    estimate2026.id,
    round2(sum(estimate2026.sections.map((section) => section.baseline.annual_gross)))
  ],
  ...estimate2026.sections.map((section): readonly [string, number] => [
    section.id,
...
const formatSignedRub = (value: number): string => `${value > 0 ? '+' : ''}${value} ₽`;

const finalGrossControl = (input: FinalGrossControlInput): EstimateDetailControlTotal => {
  const sourceRefs = detailSourceRefs(input.sourceRef);
  const aggregateTotal = getGross(aggregateGrossById, input.estimate_row_id, 'aggregate gross');
  const detailTotal = getGross(detailGrossById, input.estimate_row_id, 'detail gross');
...
export const finalControlTotals = [
  ...
  finalGrossControl({
    ...
    sourceRef: detailSource('final', 1, 'итоговая смета / строка 2.1', {
      quote: '2.1 Зимняя механизированная уборка территории; 116; 21 469 119; 87,53'
    })
  }),
  finalGrossControl({
  ...
  finalGrossControl({
    ...
    sourceRef: detailSource('final', 1, 'итоговая смета / строка 3.1', {
      quote: '3.1 Кошение травостоя вдоль открытых ливневых траншей; 105; 1 816 356; 7,40'
    })
  }),
  finalGrossControl({
  ...
  finalGrossControl({
    ...
    sourceRef: detailSource('final', 2, 'итоговая смета / строка 3.3', {
      quote: '3.3 Обработка территорий от клещей, борьба с борщевиком; 2; 6 207 773; 25,31'
    })
  }),
  finalGrossControl({
  ...
  finalGrossControl({
    ...
    sourceRef: detailSource('final', 2, 'итоговая смета / раздел 5', {
      quote: '5. Охрана и техническое обслуживание средств охраны; стоимость 14 752 949; 60,15'
    })
  }),
  finalGrossControl({
  ...
  finalGrossControl({
    ...
    sourceRef: detailSource('final', 2, 'итоговая смета / раздел 6', {
      quote: '6. Организация работы с РО по вывозу мусора; стоимость 8 004 368; 32,63'
    })
  }),
  finalGrossControl({
    ...
    sourceRef: detailSource('final', 2, 'итоговая смета / строка 6.1', {
      quote: '6.1 Организация работы с РО по вывозу мусора; 365; 8 004 368; 32,63'
    })
  }),
  finalGrossControl({
  ...
  finalGrossControl({
    ...
    sourceRef: detailSource('final', 2, 'итоговая смета / строка 7.1', {
      quote: '7.1 Техническое обслуживание уличного освещения; 12; 6 670 979; 27,20'
    })
  }),
  finalGrossControl({
    ...
    sourceRef: detailSource('final', 2, 'итоговая смета / строка 7.2', {
      quote: '7.2 Оплата электроэнергии на уличное освещение; 12; 1 473 084; 6,01'
    })
  }),
  finalGrossControl({

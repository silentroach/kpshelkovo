...
export type MeetingEntry = Pick<CollectionEntry<'meetingEntries'>, 'id' | 'data'>;
export type MeetingTranscriptEntry = Pick<CollectionEntry<'meetingTranscripts'>, 'id' | 'data'>;

let cache: Promise<MeetingsDataset> | undefined;

const TRANSCRIPT_ENTRY_ID = /^(?<id>[a-z0-9]+(?:-[a-z0-9]+)*)\/(?<part>[1-9]\d*)$/;
...
const parseTranscriptEntry = (entry: MeetingTranscriptEntry): RawMeetingTranscriptEntryInput => {
  const match = entry.id.match(TRANSCRIPT_ENTRY_ID);

  if (!match?.groups) {
    throw new Error(`meeting transcript id "${entry.id}" must use [slug]/[part]`);
  }
...
const transcriptMap = (
  ...
  transcripts.map(parseTranscriptEntry).forEach((transcript) => {
    ...
    if (parts.some((part) => part.part === transcript.part)) {
      throw new Error(`duplicate meeting transcript ${transcript.id}/${transcript.part}`);
    }
...
const buildMeetingsData = async (): Promise<MeetingsDataset> => {
  ...
  const [entries, transcripts] = await Promise.all([
    ...
    getCollection('meetingTranscripts') as Promise<readonly MeetingTranscriptEntry[]>
  ]);
...
export const loadMeeting = async (slug: string): Promise<Meeting | undefined> => {

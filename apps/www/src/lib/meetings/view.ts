...
const trimMetaDescription = (value: string): string => {
  ...
  const cut = lastSpace > 0 ? trimmed.slice(0, lastSpace) : trimmed.slice(0, textLimit);
...
export const formatMeetingDate = (date: MeetingMoment): string => {
  ...
  return date.hasTime ? `${visibleDate}, ${value.toFormat('HH:mm')}` : visibleDate;
};
...
export const describeMeeting = (meeting: Meeting): string => {
  ...
  const description =
    context.length < 80 ? `${context} ${transcriptSourcePhrase(meeting)}` : context;

  return trimMetaDescription(description);
};

export const formatTranscriptTime = (time: MeetingTranscriptTime): string => time.value;

export const formatTranscriptPartLabel = (part: Pick<MeetingTranscriptPart, 'index'>): string =>
  `Часть ${part.index}`;

export const formatMeetingSourceLabel = (sourceCount: number, index: number): string =>
  sourceCount === 1 ? 'Источник записи' : `Источник записи ${index + 1}`;

export const formatMeetingSpeakerAnchor = (speaker: Pick<MeetingSpeaker, 'id'>): string =>
  `speaker-${speaker.id}`;

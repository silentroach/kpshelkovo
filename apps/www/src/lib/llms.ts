...
const sectionLinks = () =>
  md.list([
    ...
    markdownLinkItem(
      ...
      'выбранный месяц и ссылки на дни. Краткий Markdown дня ведёт на полные сведения мероприятий с условиями участия, состоянием и источником.'
    ),
    markdownLinkItem(
      'Статус сервисов',
    ...
    markdownLinkItem(
      ...
      'индекс видимых на карте мест со ссылками на подробные карточки. В Markdown-карточках метаданные и часы работы находятся во frontmatter, редакционный текст и обратные ссылки на упоминания — в body. JSON карты содержит маркеры и геометрию.'
    ),
    markdownLinkItem(
      'Отзывы',
      ...
      llmsSection('Мероприятия по датам', [
        md.list([
          markdownLinkItem(
            ...
            'месячный календарь, краткие списки дней и подробные страницы мероприятий.'
          ),
          markdownLinkItem(
        ...
        md.paragraph(
          'Markdown дня помогает выбрать мероприятие и ведёт на его подробный Markdown с полным описанием, условиями и источником. Адрес мероприятия имеет вид /events/YYYY/MM/slug/ по месяцу начала; его Markdown находится по тому же адресу с index.md. Поле url в JSON ведёт на подробную HTML-страницу. Ссылку на ICS берите из icsUrl; одна дата без часов не имеет ICS.'
        ),
        md.paragraph(
          'Каталог содержит все редакторские записи раздела, а не весь архив Telegram или все мероприятия поблизости. Отмена сохраняется в истории; прошедшая дата не подтверждает проведение. Для многодневных записей through включает последний день; все дни ведут на одно мероприятие.'
        )
      ]),

...
let newsPublicSurfaceSlice: typeof newsPublicSurfaceSliceType & PublicSurfaceSlice;
...
beforeAll(async () => {
  ...
  ({ newsPublicSurfaceSlice, surfaceHref } = await import('@/lib/public-surface'));
});
...
describe('news discovery payload', () => {
  ...
  it('builds Link headers from registered relations, URLs, and MIME types', () => {
    ...
    const apiCatalog = newsPublicSurfaceSlice.surfaces.find((surface: PublicSurface) =>
  ...
  it('keeps the API catalog self-link intentionally absent', () => {
    ...
    expect(JSON.stringify(catalog(root))).not.toContain(surfaceHref(root, apiCatalog));
  });
  ...
  it('keeps schema, openapi, and payload contracts aligned', () => {
    ...
    const openapiDefs = api.components?.schemas?.NewsArticlesPayload?.$defs ?? {};
    const publicArticles = buildNewsPayload(
      dataset((['official', 'community', 'editorial', 'other'] as const).map(articleWithEvent))
    ).articles;
    const publicArticle = publicArticles[0];
    const publicEvent = publicArticle?.events?.[0];
    const serializedEvent = JSON.parse(JSON.stringify(publicEvent)) as Record<string, unknown>;
    const serializedOrganizer = serializedEvent.organizer as Record<string, unknown>;
    ...
    expect(api.components?.schemas?.NewsArticlesPayload?.required).toEqual(jsonSchema.required);
    expect(api.components?.schemas?.NewsArticlesPayload?.properties?.generated_at).toMatchObject({
      format: 'date-time'
    });
    expect(defs.event?.required).toEqual(['slug', 'title', 'starts_at', 'ics_url']);
    ...
    expect(
      api.paths?.['/news/data/articles.json']?.get?.responses?.[200]?.content?.['application/json']
        ?.schema?.$ref

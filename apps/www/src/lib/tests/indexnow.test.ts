...
import { submitIndexNowUrls, submitNewIndexNowPages, writeIndexNowKeyFile } from '../indexnow';
...
const writeUrlManifest = async (manifestPath: string, urls: readonly string[]): Promise<void> =>
  writeFile(manifestPath, JSON.stringify(urls));
...
describe('writeIndexNowKeyFile', () => {
  it('writes the validated UTF-8 ownership file at the site root', async () => {
    ...
    expect(await readFile(join(siteRoot, `${TEST_KEY}.txt`), 'utf8')).toBe(TEST_KEY);
  });
...
describe('submitNewIndexNowPages', () => {
  it('submits only new indexable pages after verifying the key', async () => {
    ...
    await expect(submitNewIndexNowPages(urlManifestPath, changesPath, TEST_KEY, request)).resolves
...
describe('submitIndexNowUrls', () => {
  it('keeps each POST at the 10,000 URL protocol limit', async () => {
    ...
    expect(
      request.mock.calls.map(([, init]) => JSON.parse(String(init?.body)).urlList.length)
    ).toEqual([10_000, 1]);
  });
});

...
const flattenEstimateRows = (rows: readonly EstimateRow[]): readonly EstimateRow[] =>
...
const sourceList = <T>(
  ...
  refs.flatMap((ref, index) => [...(index > 0 ? [md.text('; ')] : []), format(ref)]);

const fullSources = (refs: readonly FullReglamentSourceRef[]): MarkdownPhrasingNodes =>
  sourceList(refs, fullSource);
...
const serviceTitleById = new Map(
  fullReglamentDataset2026.services.map((service) => [service.id, service.title])
);
...
const manualCommonAssets = fullReglamentDataset2026.common_assets.filter((asset) =>
  requiresManualCheck(asset.verification_note)
);

const manualServiceMappings = fullReglamentDataset2026.service_to_estimate_map.filter(
  (item) => item.status === 'needs_check' || requiresManualCheck(item.verification_note)
);

const manualCalculationAssumptions = fullReglamentDataset2026.calculation_assumptions.filter(
  (item) => requiresManualCheck(item.status_label_ru)
);
...
} as const satisfies Record<FullReglamentServiceToEstimateMapItem['status'], string>;
...
const estimateSources = (refs: readonly EstimateSourceRef[]): MarkdownPhrasingNodes =>
  sourceList(refs, estimateSource);
...
const estimateManualLine = (row: EstimateRow): MarkdownListItem =>
  md.listItem([
    md.paragraph([
      ...
      md.text(`; причина: ${row.description ?? row.tags?.join(', ') ?? 'требует проверки'}`)
    ])
  ]);
...
const serviceManualLine = (item: FullReglamentServiceToEstimateMapItem): MarkdownListItem => {
...
const calculationManualLine = (item: FullReglamentCalculationAssumption): MarkdownListItem =>
  md.listItem([
    md.paragraph([
      md.text(
...
const auditManualLine = (item: FullReglamentAuditNote): MarkdownListItem =>
  md.listItem([
    md.paragraph([
      md.text(`audit_notes:${item.id}: ${item.title}; ${item.public_wording}; источник: `),
      ...fullSources(item.source_refs),
...
): readonly MarkdownNode[] => [md.heading(3, title), ...(rows.length > 0 ? [md.list(rows)] : [])];

const serviceLine = (service: FullReglamentService): MarkdownListItem => {
  const note = service.frequency_note ? `; примечание: ${service.frequency_note}` : '';
...
const mappingLine = (item: FullReglamentServiceToEstimateMapItem): MarkdownListItem =>
  md.listItem([
    md.paragraph([
      md.text(`${item.service_id}: ${item.status_label_ru} (`),
      md.inlineCode(item.status),
      md.text(`); строки сметы: ${item.estimate_row_ids.join(', ') || '-'}; ${item.explanation}`)
    ])
  ]);

const calculationLine = (item: FullReglamentCalculationAssumption): MarkdownListItem =>
  md.listItem([
    md.paragraph([
      md.text(
...
const topicLine = (title: string, url: string, description: string): MarkdownListItem =>
  md.listItem([md.paragraph([md.link(absoluteUrl(url), title), md.text(`: ${description}`)])]);
...
export const buildFullReglamentMarkdown = (): string =>
  serialize([
    ...
    md.list([
      ...
      row('Годовой итог', `${fullReglamentDataset2026.tariff_summary.total_annual_cost_rub} руб.`),
      row(
        'Тариф',
    ...
    md.list([
      ...
      row('Общее имущество', String(fullReglamentDataset2026.common_assets.length)),
      ...
      row('Заметки аудита', String(fullReglamentDataset2026.audit_notes.length))
    ]),
...
export const buildFullReglamentChecksMarkdown = (): string =>
  serialize([
    ...
    md.list([
      ...
      row('Строк сметы с тегом «требует проверки»', String(manualEstimateRows.length)),
      row('Объектов общего имущества с открытой проверкой', String(manualCommonAssets.length)),
      row('Сопоставлений услуг, где нужна сверка', String(manualServiceMappings.length)),
      row('Расчетных допущений, требующих проверки', String(manualCalculationAssumptions.length)),
    ...
    md.list(fullReglamentDataset2026.calculation_assumptions.map(calculationLine)),

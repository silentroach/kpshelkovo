...
import { REGLAMENT_CAVEATS, REGLAMENT_FORMULAS, buildReglamentPayload } from './discovery';
...
export function build(kind: 'short' | 'full'): string {
  ...
  const rows = estimate2026.sections.reduce((total, section) => total + rowsCount(section.rows), 0);
  ...
  const fullServiceMapMarkdown = absoluteUrl(reglamentFullServiceMapMarkdownUrl());
  ...
  const detailsMaterialsMarkdown = absoluteUrl(reglamentEstimateDetailsMaterialsMarkdownUrl());
  const detailsMachinesMarkdown = absoluteUrl(reglamentEstimateDetailsMachinesMarkdownUrl());
  const detailsLaborMarkdown = absoluteUrl(reglamentEstimateDetailsLaborMarkdownUrl());
  const detailsChecksMarkdown = absoluteUrl(reglamentEstimateDetailsChecksMarkdownUrl());
        ...
        sections: [
          ...
          llmsSection('Как читать JSON', [
            markdownList([
              ...
              'В `estimate-details-2026.json` поле `sources` хранит уникальные фрагменты PDF, а `source_refs` у фактов и `needs_check` — их идентификаторы.',
        ...
        sections: [
          ...
          llmsSection('Канонические URL', [
            markdownList([
              ...
              `JSON Schema: ${schema}`,
              `OpenAPI: ${openapi}`,
              ...payload.sources.map((item) => `PDF ${item.pdf}.pdf: ${absoluteUrl(item.pdf_url)}`)
            ])
          ]),
          llmsSection('JSON', [
            markdownList([
              ...
              'В `estimate-details-2026.json` поле `sources` хранит уникальные фрагменты PDF, а `source_refs` у работ, ресурсов, контрольных итогов и `needs_check` — их идентификаторы.',

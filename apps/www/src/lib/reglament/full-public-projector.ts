...
const projectSourceRef = (sourceRef: FullReglamentSourceRef): PublicFullReglamentSourceRef => ({
  pdf: sourceRef.pdf,
  page: sourceRef.page,
...
const projectVillage = (village: FullReglamentVillage): PublicFullReglamentVillage => ({
  id: village.id,
  title: village.title,
...
const projectCommonAsset = (asset: FullReglamentCommonAsset): PublicFullReglamentCommonAsset => ({
  id: asset.id,
  category: asset.category,
  title: asset.title,
  unit: asset.unit,
  values_by_village: {
    'shelkovo-village': projectQuantity(asset.values_by_village['shelkovo-village']),
    'shelkovo-forest': projectQuantity(asset.values_by_village['shelkovo-forest']),
    'shelkovo-park': projectQuantity(asset.values_by_village['shelkovo-park']),
    'shelkovo-river': projectQuantity(asset.values_by_village['shelkovo-river'])
  },
...
const projectService = (service: FullReglamentService): PublicFullReglamentService => ({
  id: service.id,
  group: service.group,
  title: service.title,
...
const projectAuditNote = (auditNote: FullReglamentAuditNote): PublicFullReglamentAuditNote => ({
  id: auditNote.id,
...
export const projectPublicFullReglamentDataset = (
  ...
  if (dataset.schema_version !== '1' || dataset.dataset_id !== 'full-reglament-2026') {
  ...
  return {
    ...
    tariff_summary: {
      ...
      tariff_rub_per_sotka_month: dataset.tariff_summary.tariff_rub_per_sotka_month,
    ...
    service_to_estimate_map: dataset.service_to_estimate_map.map(projectServiceToEstimateMapItem),
    calculation_assumptions: dataset.calculation_assumptions.map(projectCalculationAssumption),

...
export { buildPeopleDataset, loadPeopleData, loadPeopleMentionRegistry } from './registry';
...
const buildPeopleDataWithBacklinks = async (): Promise<PeopleDataset> => {
  const [people, graph] = await Promise.all([loadPeopleData(), loadSiteMentionGraph()]);
...
export const loadPeopleProfilesWithBacklinks = async (): Promise<readonly PersonProfile[]> =>
  (await loadPeopleDataWithBacklinks()).profiles;

export const loadPersonProfile = async (slug: string): Promise<PersonProfile | undefined> => {
  const key = slug.trim();
...
export const loadPersonProfileWithBacklinks = async (
  ...
  return key ? (await loadPeopleDataWithBacklinks()).bySlug.get(key) : undefined;
};

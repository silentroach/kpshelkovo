...
export const buildSiteMentionRegistry = (
  ...
  createSiteMentionRegistry([
    ...
    ...places.map((entry) =>
      createPlaceMentionTarget(entry.id, entry.data.title, entry.data.name_cases)
    )
  ]);

export const loadSiteMentionRegistry = (): Promise<SiteMentionRegistry> => {
  cache ??= Promise.all([getCollection('peopleProfiles'), getCollection('places')]).then(
    ([people, places]) => buildSiteMentionRegistry(people, places)
  );

...
const targetKey = (type: EntityMentionType, slug: string): string => `${type}:${slug}`;
...
const compareRefs = (a: EntityMentionSourceRef, b: EntityMentionSourceRef): number =>
  (b.sortKey ?? Number.NEGATIVE_INFINITY) - (a.sortKey ?? Number.NEGATIVE_INFINITY) ||
...
const assertNotSelfLink = (ref: EntityMentionSourceRef): void => {
  if (ref.sourceEntity?.type === ref.target.type && ref.sourceEntity.slug === ref.target.slug) {
    throw new Error(`self-link mention ref ${targetKey(ref.target.type, ref.target.slug)}`);
  }
};

const createGraphTarget = (type: EntityMentionType, slug: string): EntityMentionGraphTarget => ({
  type,
  slug,
  sections: new Map()
});
...
export const createEntityMentionGraph = (
  ...
  for (const ref of refs) {
    ...
    const target = targets.get(key) ?? createGraphTarget(ref.target.type, ref.target.slug);

...
const RawKbPageSourceSchema = z
  .object({
    url: z.string().trim().min(1, 'sources[].url must not be blank').refine(isKbSourceUrl, {
      message: 'sources[].url must be an http(s) URL or a root-relative site path'
    }),
    description: z.string().trim().min(1, 'sources[].description must not be blank')
  })
  .strict();
...
const RawKbPageSourcesSchema = z
  ...
  .refine((sources) => new Set(sources.map(({ url }) => url)).size === sources.length, {

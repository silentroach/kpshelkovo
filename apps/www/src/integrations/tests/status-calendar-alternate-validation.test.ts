...
const writeCalendarHtml = async (directory: string, href: string): Promise<void> => {
...
describe('status calendar alternate validation', () => {
  it('accepts an alternate backed by a physical Markdown document', async () => {
    ...
    await writeCalendarHtml(directory, 'https://kpshelkovo.online/status/calendar/2026/index.md');
    await writeFile(join(directory, 'status/calendar/2026/index.md'), '# Календарь');
  ...
  it('reports a declared alternate missing from the build', async () => {
    ...
    await expect(validateStatusCalendarAlternates(pathToFileURL(`${directory}/`), site)).rejects

...
Тест проверяет заполненный индикатор 8px рядом с заголовком, его `title`, скрытую
live-подпись и отсутствие горизонтального переполнения страницы. После снимка
варианта с перерывом часы переводятся к границам интервалов: открытый статус
сообщает конец текущего интервала (13:00 или 18:00), закрытый — «Сейчас закрыто».
Сокращения дней строчные; «Ежедневно» сохраняет прописную букву.
Список использует две общие колонки: дни без двоеточий занимают `max-content`,
время всех строк начинается на одной координате x. После mobile-снимка сценарий
с двумя интервалами проверяется на ширине 260 px: второй интервал переносится
внутри правой колонки, каждый интервал остаётся целым, горизонтального переполнения нет.

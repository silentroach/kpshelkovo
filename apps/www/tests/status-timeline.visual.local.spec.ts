...
test.describe('StatusServiceTimeline visual', () => {
  ...
  test('keeps problem segments touch-friendly while the visible marker stays compact', async ({
    ...
    const metrics = await target.locator('[data-status-problem]').evaluateAll((segments) =>
      segments.map((segment) => {
        ...
        const rangeDays = Number(root instanceof HTMLElement ? root.dataset.rangeDays : '');
    ...
    expect(shortActiveMetric.markerWidth).toBeLessThanOrEqual(shortActiveMetric.dayWidth * 1.05);
  });

  test('renders mixed timeline with hydrated green gaps and open tooltip', async ({ page }) => {
    const target = page.getByTestId('status-timeline-mixed');

    await openTimelineTooltip(page, target, 'mixed-incident');

    await expect(target.locator('[data-status-segment="green"]')).toHaveCount(3);

    await expect(target).toHaveScreenshot('status-timeline-mixed-tooltip.png', screenshot);
  });

  test('renders a full green bar when there are no incidents', async ({ page }) => {
    const target = page.getByTestId('status-timeline-empty');

    await expect(target.locator('[data-status-problem]')).toHaveCount(0);

    await expect(target).toHaveScreenshot('status-timeline-empty.png', screenshot);
  });

  test('keeps a short active incident fully visible at the right edge', async ({ page }) => {
    const target = page.getByTestId('status-timeline-active-edge');

    await expect(target.locator('[data-status-problem]')).toHaveCount(1);

    await expect(target).toHaveScreenshot('status-timeline-active-edge.png', screenshot);
  });

  test('groups dense problem segments near the right edge', async ({ page }) => {
    const target = page.getByTestId('status-timeline-dense-problems');

    await expect(target.locator('[data-status-problem]')).toHaveCount(1);

    await expect(target).toHaveScreenshot('status-timeline-dense-problems.png', screenshot);
  });

  test('renders grouped tooltip rows with the same typography pattern', async ({ page }) => {
    const target = page.getByTestId('status-timeline-dense-problems');
    ...
    await expect(target).toHaveScreenshot('status-timeline-dense-problems-tooltip.png', screenshot);
  });
});

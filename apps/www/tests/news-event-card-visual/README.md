...
Локальный fixture для screenshot tests общего `EventWidget` в новости и подробной странице `EventCard`.
...
на desktop (1440px) и mobile (390px). На desktop ширина обеих карточек равна 19rem,
...
рядом с компактной кнопкой OpenMaps.

Fixture передаёт доменный `NewsEvent` из `lib/news/types` с camelCase-полями и
`satisfies NewsEvent`. Один `EventWidget` показывает дату, название, время, место и ICS
в обоих размещениях. В новости название ведёт на мероприятие, на подробной странице это обычный текст.
Отдельная проверка сопоставляет настройки карты, появление подложки до кнопки без ожидания тайлов,
сохранение фокуса на ссылке места, единственное верхнее действие и приглушение атрибуции.
Отмена проверяется красной зачёркнутой датой без отдельной иконки, с доступным именем
и без скачивания ICS.

## Внешняя карта

До навигации Playwright блокирует `https://api-maps.yandex.ru/**` и устанавливает
через `addInitScript` минимальный mock SDK.
Настоящий API-ключ не требуется. Вместо тайлов
он вставляет `map-background.svg` и ссылки copyright из `#map-sdk-fixture`,
а кнопку OpenMaps — после отдельного сигнала готовности контрола из `#map-control-fixture`.
Настоящий `MapPreview` клонирует маркер из своего `template`
...
copyright снизу справа по переданным параметрам SDK и компактную кнопку сверху справа.
Wrapper маркера — обычный
...
ссылку места и ICS; резервной ссылки на внешнюю карту нет. Штатные действия должны получать

...
import { isAbsoluteUrl, isDomylandUrl, isTelegramUrl, telegram, withBase } from './index';
...
describe('withBase package', () => {
  it('passes through external and special URLs', () => {
    expect(withBase('/base/', 'https://example.com')).toBe('https://example.com');
    expect(withBase('/base/', '#map')).toBe('#map');
    expect(withBase('/base/', 'mailto:test@example.com')).toBe('mailto:test@example.com');
    expect(withBase('/base/', 'tel:+79990000000')).toBe('tel:+79990000000');
    expect(withBase('/base/', '//cdn.example.com/app.js')).toBe('//cdn.example.com/app.js');
  });

  it('prepends normalized base to relative paths', () => {
    expect(withBase('/base/', 'settlements/lesnoe/')).toBe('/base/settlements/lesnoe/');
  });

  it('treats unsupported URI schemes as internal paths', () => {
    expect(withBase('/base/', 'javascript:alert(1)')).toBe('/base/javascript:alert(1)');
    expect(withBase('/base/', 'data:text/plain,hello')).toBe('/base/data:text/plain,hello');
  });

  it('prepends normalized base to absolute internal paths', () => {
    expect(withBase('/base/', '/settlements/usadby/')).toBe('/base/settlements/usadby/');
  });

  it('works without base', () => {
    expect(withBase(undefined, 'settlements/lesnoe/')).toBe('/settlements/lesnoe/');
  });
});

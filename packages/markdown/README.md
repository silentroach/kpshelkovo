...
- `formatPlainText(text)`
  Типографирует обычный текст на сборке без DOM, используя правила сайта. HTML-, Markdown- и mention-подобные строки остаются текстом; буквальные HTML-сущности сохраняются. Результат предназначен для `textContent` или экранирующего шаблона, а не для прямой вставки HTML.

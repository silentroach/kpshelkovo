...
- `/news/` показывает свежие и закрепленные публикации и ведет в последний доступный годовой архив.
- `/news/YYYY/` служит каталогом непустых месяцев года и связывает доступные годовые архивы между собой.
- `/news/YYYY/MM/` показывает публикации месяца от новых к старым и ведет к соседним месяцам, в том числе через границу года.
- HTML и Markdown companions сохраняют одинаковую структуру архива: год выбирает месяц, месяц перечисляет публикации.
